<script lang="ts">
  ...
  $: filteredTodos = todos.filter((todo) => {
    // only return items that contain the search string (case insensitive)
    const lowerCaseSearchString = searchString.toLowerCase();
    const lowerCaseTodoValue = todo.value.toLowerCase();
    return lowerCaseTodoValue.includes(lowerCaseSearchString);
  });
